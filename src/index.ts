...
class Bank {
  ...
  public createAccount(
    ...
    if (!this.accounts.has(clientId)) {
      this.accounts.set(clientId, []);
    }
    this.accounts.get(clientId)?.push(account);

    return account;
  }

  public closeAccount(account: BankAccount): void {
    const clientId = this.getClientId(account.holder());
    const clientAccounts = this.accounts.get(clientId);

    if (clientAccounts) {
      const index = clientAccounts.indexOf(account);
      if (index !== -1) {
  ...
  private getClientId(client: IBankClient): string {
...
const johnAccountUSD = bank.createAccount(
  { firstName: 'John', lastName: 'Doe' },
  CurrencyTypesEnum.USD,
  currentRateStrategy
);
const johnAccountEUR = bank.createAccount(
  { firstName: 'John', lastName: 'Doe' },
  CurrencyTypesEnum.EUR,
  fixedRateStrategy
);
const janeAccountUSD = bank.createAccount(
  { firstName: 'Jane', lastName: 'Smith' },
  CurrencyTypesEnum.USD,
  currentRateStrategy
);

bank.closeAccount(johnAccountUSD);
bank.closeAccount(johnAccountEUR);
bank.closeAccount(janeAccountUSD);
